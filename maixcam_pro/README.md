...
- `1`：上游 `v2.1.0-cut-topologies` 多拓扑邻接图算法，实现在
  `puzzle_solver.py`，当前默认启用；它会直接从轮廓自动搜索完整边、T 形部分边、
  凹多边形和相同矩形，不依赖仿真生成器提供切割类别；
- `2`：此前的逐轮轮廓合并算法，保留在 `puzzle_solver_merge.py`，用于现场对比。

算法 1 在上游固定 10 cm × 6 cm 评分的基础上做了真机适配：目标面积从检测到
的碎片面积计算，目标位置仍由 A4 透视坐标决定，并保留 `MIN_RECTANGLE_FILL`
硬门限。这样不会引用仿真画布坐标或碎片生成真值。

兼容算法 2 不枚举“第三刀”或猜测切割过程，而是从检测到的 1～4 个轮廓直接工作：
