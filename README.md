...
这个项目生成一张随机切成 4 个多边形的矩形卡纸，把碎片随机旋转、平移并无重叠地摆放在 A4 纸上半区，然后**只从渲染图像**完成：
...
切割模型是在矩形内部随机取一点，再分别连接四条外边上的随机点。这样得到的 4 块均为四边形，且每块都有矩形外边，符合赛题“不超过 4 片、每片不超过 5 边、每片至少一条边位于目标矩形外边”的约束。
